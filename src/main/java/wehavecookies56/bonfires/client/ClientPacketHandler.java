...
public class ClientPacketHandler {
    ...
    public static void openBonfire(OpenBonfireGUI packet) {
        MinecraftClient.getInstance().setScreen(new BonfireScreen((BonfireTileEntity) MinecraftClient.getInstance().world.getBlockEntity(packet.tileEntity), packet.ownerNames, packet.dimensions.stream().filter(dim -> !Bonfires.CONFIG.client.hiddenDimensions().contains(dim.getValue().toString())).toList(), packet.registry, packet.canReinforce));
    }
