...
public class ScrollBarButton extends Button {
    ...
    public void render(PoseStack stack, int mouseX, int mouseY, float partialTicks) {
        if (visible) {
            ...
            fill(stack, x, y, x + width, y + height, new Color(0, 0, 0, 0.5F).hashCode());
            RenderSystem.disableBlend();
            RenderSystem.setShaderColor(1, 1, 1, 1);
            fill(stack, x, handleY, x+8, handleY+handleHeight, new Color(81, 86, 71).hashCode());
            fill(stack, x+1, handleY+1, x+1+6, handleY+1+(handleHeight-2), new Color(114, 118, 95).hashCode());
        }
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (button == 0) {
            ...
            startX = x;
            startY = handleY;
            if (clickY >= y && clickY <= y + height && clickX >= x && clickX <= x + width && visible) {
