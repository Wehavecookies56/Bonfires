...
public class ScrollBarButton extends Button {
    ...
    public void render(PoseStack stack, int mouseX, int mouseY, float partialTicks) {
        if (visible) {
            ...
            fill(stack, getX(), getY(), getX() + width, getBottom(), new Color(0, 0, 0, 0.5F).hashCode());
            RenderSystem.disableBlend();
            RenderSystem.setShaderColor(1, 1, 1, 1);
            fill(stack, getX(), handleY, getX()+8, handleY+handleHeight, new Color(81, 86, 71).hashCode());
            fill(stack, getX()+1, handleY+1, getX()+1+6, handleY+1+(handleHeight-2), new Color(114, 118, 95).hashCode());
        }
    }
